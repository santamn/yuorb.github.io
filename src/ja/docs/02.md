...
## 2.2 母音形式への声門閉鎖音の挿入規則 {#Sec2_2}

この言語の語形成について見ていくと、形成詞を構成する槽の一部において、母音形式 $\bm{V}$ への声門閉鎖音の挿入を要求されることがあります。その場合は、以下の規則に従ってください。

1. $\bm{V}$ が単母音または二重母音の場合、$\bm{V}$ の後ろに声門閉鎖音を配置する（例: -**a** は -**a’** に、-**ai** は -**ai’** になる）
2. $\bm{V}$ が二音節接続の場合、$\bm{V}$ の二つの母音の間に声門閉鎖音を挿入する（例: -**ua** は -**u’a** になる）
3. 上記の規則1を適用すると、その挿入によって音素配列的に許容されない、または音便上望ましくない結合が生じる、あるいは声門破裂音が語末に来てしまう場合、次のように挿入母音を追加する
    * $\bm{V}$ が単母音の場合: 声門破裂音の後にその母音を重ねる（例: -**a** は -**a’a** になる）
    * $\bm{V}$ が二重母音の場合: 二重母音の二つの母音の間に声門閉鎖音を配置する（規則1の例外。-**ai** は通常の -**ai’** ではなく -**a’i** となる）
4. 単語の音節数を減らす目的で、特定の状況下で第9槽の $\bm{V_C}$ 接辞の中にある声門閉鎖音を単語の別の槽へ移動させる方法については、[第4.6節の特記事項](04#Sec4_6)で説明されている

## 2.3 形成詞の構造 {#Sec2_3}

形成詞の形態的構造は、以下の式で表すことができます。

::: center

<code>$\bm{(C_C + V_V) + C_R + V_R + (C_SV_X \ldots) + C_A + (V_XC_S \ldots) + (V_NC_N) + V_C / V_K + [強勢]}$</code>

:::

ここで、$\bm{C_R}$ と $\bm{[強勢]}$ を除く各項は、子音形式（式中では $\bm{C}$ と表記）や母音形式（ $\bm{V}$ と表記）、あるいはそれらを組み合わせたもの（例: $\bm{C_S V_X}$ や $\bm{V_N C_N}$ ）から成る接辞を表しています。$\bm{C_R}$ という項は語根そのものを指していて、これは1〜5個の子音で構成される子音形式です。括弧書きになっている項は省略可能であり、最小構成の形成詞は $\bm{C_R + V_R + C_A + V_C / V_K + [強勢]}$ という5つの項だけで構成されています。これらの様々な形態的要素は現れる順序が厳密に決まっているため、形成詞を解析する際は、それぞれの要素がこれら10個の形態的な『槽』のいずれかを埋めている、という形で分析できます。以下の表に示すように、これらの槽には前から順に第1槽から第10槽までの名前がついています。
